...
export default function HomeMenuSection() {
  ...
  return (
    <section 
      id="menu" 
      className={`py-12 sm:py-16 md:py-20 bg-background-warm ${isRTL ? 'rtl' : 'ltr'}`}
      dir={isRTL ? 'rtl' : 'ltr'}
    >
      <div className="container mx-auto px-3 sm:px-4 lg:px-8 max-w-7xl">
        {/* Section Header */}
        <div className={`text-center mb-12 sm:mb-16 ${isRTL ? 'text-right' : 'text-left'} lg:text-center`}>
          <div className="inline-block mb-3 sm:mb-4">
            <span className="bg-gradient-to-r from-primary to-accent-red text-white px-4 sm:px-6 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium shadow-warm">
              {t('homeMenu.subtitle')}
            </span>
          </div>
          
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-text-heading mb-4 sm:mb-6 leading-tight px-2">
            <span className="bg-gradient-to-r from-primary to-accent-red bg-clip-text text-transparent">
              {t('homeMenu.title')}
            </span>
          </h2>
          
          <p className="text-text-light text-base sm:text-lg max-w-3xl mx-auto px-4">
        ...
        <div className="text-center px-2 sm:px-4">
          <div className="bg-white rounded-xl sm:rounded-2xl p-6 sm:p-8 shadow-warm max-w-2xl mx-auto">
            <h3 className="text-xl sm:text-2xl font-bold text-text-heading mb-3 sm:mb-4">
              {t('homeMenu.ctaTitle')}
            </h3>
            <p className="text-text-light text-sm sm:text-base mb-5 sm:mb-6">
              {t('homeMenu.ctaDescription')}
            </p>
            <Link 
              href="/menu"
              className="inline-block bg-primary hover:bg-secondary-dark text-white font-semibold py-3 px-6 sm:px-8 text-sm sm:text-base rounded-custom transition-all duration-300 shadow-warm hover:shadow-hover transform hover:-translate-y-1 w-full sm:w-auto"
            >
